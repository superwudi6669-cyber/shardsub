...
from .video_io import get_video_info, iter_sampled_frames
...
class SubtitleExtractor:
    ...
    def extract(
        ...
        # Fallback metadata in case no frame is sampled (e.g. zero-length video);
        # overwritten by the per-frame info below when frames are available.
        info = get_video_info(video_path)
        started = time.perf_counter()

        for frame_index, time_sec, frame, info in iter_sampled_frames(
            video_path,
            self.config.extract.ocr_every_n_frames,
        ):
